...
def validateDeck(args):
    mute()
    characterCount = len(me.piles["Character"])
    if characterCount == 2:
        notify("{} loads a valid two-card Character section.".format(me))
    else:
        whisper("Your Character section must contain exactly 2 cards; it currently contains {}.".format(characterCount))
        notify("{} loaded a deck with {} cards in the Character section (exactly 2 required).".format(me, characterCount))
